...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (status === 'loading') {
    return (
      <div className="admin-layout min-h-screen flex items-center justify-center" style={{ background: '#0a0a0a' }}>
  ...
  return (
    ...
    <div className="admin-layout min-h-screen flex" style={{ background: '#0a0a0a' }}>
      {/* Sidebar */}
      <aside
        className="w-60 flex-shrink-0 flex flex-col py-8 px-5"
      ...
      >
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="flex items-center gap-3 px-3 py-2.5 text-sm tracking-wide transition-all duration-150 relative"
                style={{
                  color: active ? 'var(--accent)' : 'var(--muted)',
                  background: active ? 'rgba(200,246,94,0.06)' : 'transparent',
        ...
        <div className="pt-6 border-t" style={{ borderColor: 'var(--border)' }}>
          ...
          <button
            ...
            className="w-full text-left px-3 py-2 text-xs tracking-widest uppercase transition-colors duration-150 hover:text-red-400"
